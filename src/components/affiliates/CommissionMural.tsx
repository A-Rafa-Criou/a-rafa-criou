                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {isPromotion && (
                            <p className="text-xs text-muted-foreground ml-auto max-w-30 text-right">
                                Taxa promocional! Aproveite para vender mais!
                            </p>
                        )}
                    </div>
                </CardContent>
            ...
            {isPromotion && (
                <div className="absolute top-0 right-0">
                    <Badge className={`rounded-none rounded-bl-xl text-sm font-bold px-4 py-2 animate-pulse ${isHighPromotion ? 'bg-red-500 text-white' : 'bg-[#FD9555] text-white'
                        ...
                        {isHighPromotion ? 'SUPER PROMOÇÃO!' : 'PROMOÇÃO!'}
                    </Badge>
                </div>
            )}
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
                    ...
                    <div className="text-center sm:text-left flex-1">
                        <p className="text-xs sm:text-sm text-muted-foreground font-semibold uppercase tracking-wider mb-1">
                            {isPromotion ? 'Taxa Promocional de Comissão' : 'Sua Taxa de Comissão'}
                        </p>
                        ...
                        <p className="text-sm text-muted-foreground mt-2 max-w-md">
                            {isHighPromotion
                                ? 'Aproveite esta oportunidade incrível! Ganhe mais em cada venda que realizar.'
                                : isPromotion
                                    ? 'Comissão aumentada por tempo limitado! Venda mais e ganhe mais.'
                                    : 'Você recebe esta porcentagem sobre cada venda realizada através do seu link de afiliado.'
                            }
                        </p>
                    </div>
                    ...
                    <div className={`hidden lg:flex flex-col items-center gap-1 px-6 py-3 rounded-xl ${isPromotion ? 'bg-white/60' : 'bg-white/50'
                        ...
                        <span className="text-xs text-muted-foreground font-medium">Exemplo</span>
                        <span className="text-xs text-muted-foreground">Venda de R$ 100</span>
                        <span className={`text-2xl font-bold ${isHighPromotion ? 'text-red-600' : isPromotion ? 'text-[#FD9555]' : 'text-green-600'
                            }`}>
                            R$ {rateNum.toFixed(0)}
                        </span>
                        <span className="text-xs text-muted-foreground">para você</span>
                    </div>
                </div>
            </CardContent>
        </Card>
